...
class PrettyHandsome_Plugin implements PluginInterface
{
    ...
    public static function config(Form $form)
    {
        ...
        // 设置静态背景图片开关
        $form->addInput(PluginsForm::StaticBackgroundImage());

        // 设置静态背景图片地址
        $form->addInput(PluginsForm::StaticBackgroundImageUrl());

        // 动态背景
        $form->addInput(PluginsForm::DynamicBackground());

        // 移动端隐藏动态背景
        $form->addInput(PluginsForm::MobileHideDyBackground());

        // 夜间模式隐藏动态背景
        $form->addInput(PluginsForm::ThemeDarkHideDyBackground());

        // 评论头像呼吸效果
        $form->addInput(PluginsForm::CommentAvatarBreath());

        // 文章前广告位
        $form->addInput(PluginsForm::BeforePostContentAD());

        // 评论前广告位
        $form->addInput(PluginsForm::BeforeCommentListAD());
    }
